...
from .utils import Expression, AvgPool2dWithConv, Ensure4d, identity, transpose_time_to_spat, squeeze_final_output, np_to_th


class DeepConvNet(nn.Module):
    """
    Deep learning with convolutional neural networks for EEG decoding and visualization.
    Schirrmeister, R. T., Springenberg, J. T., Fiederer, L. D. J., Glasstetter, M., Eggensperger, K., Tangermann, M., Hutter, F. & Ball, T. (2017).
    https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5655781/
    https://github.com/braindecode/braindecodes
    """
    def __init__(self, 
        num_channel, 
        num_classes, 
        ...
        self.input_window_samples = signal_length
        ...
        if self.split_first_layer:
            self.model.add_module("dimshuffle", Expression(transpose_time_to_spat))
        ...
        self.model.add_module("conv_nonlin", Expression(self.first_nonlin))
        self.model.add_module(
            "pool",
            first_pool_class(
        ...
        self.model.add_module("pool_nonlin", Expression(self.first_pool_nonlin))

        def add_conv_pool_block(
            model, n_filters_before, n_filters, filter_length, block_nr
        ):
            suffix = "_{:d}".format(block_nr)
            self.model.add_module("drop" + suffix, nn.Dropout(p=self.drop_prob))
            self.model.add_module(
                "conv" + suffix,
                nn.Conv2d(
                    n_filters_before,
            ...
            self.model.add_module("nonlin" + suffix, Expression(self.later_nonlin))
        ...
        # self.model.add_module('drop_classifier', nn.Dropout(p=self.drop_prob))
        self.eval()
        ...
        self.fc.add_module("squeeze", Expression(squeeze_final_output))
        ...
        init.xavier_uniform_(self.fc.conv_classifier.weight, gain=1)
        init.constant_(self.fc.conv_classifier.bias, 0)
